...
app.post('/api/lands', async (req, res) => {
  try {
    const { farmer_id, land_name, area_acres, soil_type, survey_no } = req.body;
    const result = await sb('POST', 'lands', { farmer_id, land_name, area_acres, soil_type, survey_no });
    res.status(201).json(result[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/lands/:id', async (req, res) => {
  try {
    const { farmer_id, land_name, area_acres, soil_type, survey_no } = req.body;
    const result = await sb('PATCH', 'lands', { farmer_id, land_name, area_acres, soil_type, survey_no }, `?id=eq.${req.params.id}`);
    res.json(result[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/crops', async (req, res) => {
  try {
    const { land_id, variety, season, planting_date, harvest_date, notes } = req.body;
    const result = await sb('POST', 'crops', { land_id, variety, season, planting_date: planting_date || null, harvest_date: harvest_date || null, notes });
    res.status(201).json(result[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/crops/:id', async (req, res) => {
  try {
    const { land_id, variety, season, planting_date, harvest_date, notes } = req.body;
    const result = await sb('PATCH', 'crops', { land_id, variety, season, planting_date: planting_date || null, harvest_date: harvest_date || null, notes }, `?id=eq.${req.params.id}`);
    res.json(result[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/productions', async (req, res) => {
  try {
    const { crop_id, yield_tons, price_per_ton, harvest_date } = req.body;
    const result = await sb('POST', 'productions', { crop_id, yield_tons, price_per_ton, harvest_date: harvest_date || null });
    res.status(201).json(result[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/productions/:id', async (req, res) => {
  try {
    const { crop_id, yield_tons, price_per_ton, harvest_date } = req.body;
    const result = await sb('PATCH', 'productions', { crop_id, yield_tons, price_per_ton, harvest_date: harvest_date || null }, `?id=eq.${req.params.id}`);
    res.json(result[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
